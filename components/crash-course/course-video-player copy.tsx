...
import { Play, Pause, Volume2, Volume1, VolumeX, SkipForward, SkipBack, Settings, Loader, Maximize, Minimize, Subtitles } from "lucide-react"
...
useEffect(() => {
  if (isReady && playerInstanceRef.current && videoId) {
    ...
    // Set default quality after video loads
    setTimeout(() => {
      try {
        playerInstanceRef.current.setPlaybackQuality('auto');
        setCurrentQuality('auto');
      } catch (error) {
        console.log('Could not set default quality:', error);
      }
    }, 1000);
 ...
 const onPlayerReady = (event: any) => {
  ...
  // Force quality list refresh if it's empty
  if (!qualities || qualities.length === 0) {
    setTimeout(() => {
      const refreshedQualities = event.target.getAvailableQualityLevels();
      setAvailableQualities(refreshedQualities && refreshedQualities.length > 0 ? refreshedQualities : ["auto"]);
    }, 1000);
  } else {
    setAvailableQualities(qualities);
  }

  // Get current quality and set it in state
  const currentQual = event.target.getPlaybackQuality();
  setCurrentQuality(currentQual || 'auto');

  // Set default quality to auto if not already set
  if (!currentQual || currentQual === 'unknown') {
    setTimeout(() => {
...
const simpleQualityChange = (event, quality) => {
   if (!playerInstanceRef.current) return;
  ...
  // For better quality switching, pause and seek to current time
  if (wasPlaying) {
    playerInstanceRef.current.pauseVideo();
  }
  
  // Brief delay then seek to maintain position
  setTimeout(() => {
  ...
  // Default quality options if API doesn't return any
  const displayQualities = availableQualities.length > 0 
    ? availableQualities 
  ...
  return (
    ...
    >
      ...
      {/* Custom controls */}
      <div 
        className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent px-2 sm:px-4 pb-2 sm:pb-4 pt-6 transition-opacity duration-300 z-30 ${
                  ...
                  >
                    ...
                    {(activeSettingsTab === 'quality' || !activeSettingsTab) && (
                      <div className="grid grid-cols-2 gap-1">
                        {displayQualities && displayQualities.map((quality) => (
                          <button
                            key={quality}
