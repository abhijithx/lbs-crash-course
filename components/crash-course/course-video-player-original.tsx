...
export default function CourseVideoPlayer({ videoId }: CourseVideoPlayerProps) {
  ...
  const onPlayerReady = (event: any) => {
    ...
    // Set higher quality preference and network priority
    setTimeout(() => {
      const availableQuals = event.target.getAvailableQualityLevels() || []
      let preferredQuality = "auto"

      // Prioritize highest available quality for better experience
      ...
      // Set quality and suggest network priority
      event.target.setPlaybackQuality(preferredQuality)
      setCurrentQuality(preferredQuality)

      // Request higher network priority for video loading
      if ("connection" in navigator) {
        try {
          // Suggest high priority for video content
          event.target.getIframe().style.willChange = "transform"
        } catch (e) {
          console.log("Network priority hint not supported")
        }
      }
    }, 1000)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`absolute top-0 left-0 right-0 ${
            !isPlaying
              ? "bg-gradient-to-b from-black via-black/90 to-black/60"
              : "bg-gradient-to-b from-black/70 to-transparent"
          } h-12 sm:h-16 transition-all duration-300 z-25`}
        ></div>
        ...
        <div className="absolute bottom-2 right-2 z-20 pointer-events-none">
          <div className="text-white/40 text-xs font-light">cetmca26.live</div>
        </div>
      </div>
      ...
      {/* Custom cetmca26 Loading overlay */}
      {isBuffering && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-20">
          <div className="flex flex-col items-center space-y-4">
            <div className="relative">
              <div className="animate-spin rounded-full h-12 w-12 sm:h-16 sm:w-16 border-4 border-blue-500/30 border-t-blue-500"></div>
              <div className="absolute inset-0 animate-pulse rounded-full h-12 w-12 sm:h-16 sm:w-16 border-2 border-blue-400/50"></div>
            </div>
            <div className="text-white font-semibold text-sm sm:text-base tracking-wider animate-pulse">cetmca26</div>
      ...
      <div
        ...
        } px-3 sm:px-6 pb-3 sm:pb-5 pt-16 transition-all duration-300 z-30 ${
      ...
      >
        ...
        <div className="mb-3 sm:mb-5">
          <div
            className="relative h-2 sm:h-2.5 bg-white/20 rounded-full cursor-pointer backdrop-blur-sm"
            onClick={(e) => {
          ...
          >
            <div
              className="absolute h-full bg-gradient-to-r from-blue-500 to-blue-400 rounded-full shadow-lg"
            ...
            >
              <div className="w-3 h-3 sm:w-4 sm:h-4 bg-white rounded-full shadow-lg border-2 border-blue-500"></div>
            </div>
          </div>
          <div className="flex justify-between text-xs sm:text-sm text-white/90 mt-2 font-medium">
        ...
        {isMobile ? (
          <div className="mobile-controls">
            <div className="mobile-controls-left space-x-1">
              <button
                className="text-white p-2 rounded-full hover:bg-white/20 transition-all duration-200 backdrop-blur-sm bg-black/20"
                onClick={togglePlay}
            ...
            <div className="mobile-controls-right space-x-1">
              <div className="relative volume-container">
                ...
                {showVolumeSlider && (
                  <div className="absolute bottom-full left-0 mb-2 bg-black/80 backdrop-blur-md rounded-lg p-3 w-36 border border-white/10">
                    <input
                      type="range"
                      min="0"
                      max="100"
                    ...
                    />
                    <button
                      className="text-white text-xs mt-2 w-full py-2 px-3 bg-blue-600 hover:bg-blue-700 rounded-md transition-colors"
                      onClick={toggleMute}
                    >
                      {isMuted ? "Unmute" : "Mute"}
                    </button>
              ...
              <div className="relative">
                ...
                {showSettings && (
                  <div className="absolute right-0 bottom-full mb-2 bg-black/90 backdrop-blur-md rounded-lg p-2 w-48 z-50 border border-white/10 shadow-xl max-h-60 overflow-y-auto">
                    <div className="flex mb-2 space-x-1 bg-white/10 rounded-lg p-1">
                      <div
                        className={`settings-tab flex-1 text-center ${activeSettingsTab === "quality" || !activeSettingsTab ? "bg-blue-600 text-white" : "text-white/70 hover:text-white"}`}
                        onClick={() => setActiveSettingsTab("quality")}
                      >
                        Quality
                      </div>
                      <div
                        className={`settings-tab flex-1 text-center ${activeSettingsTab === "speed" ? "bg-blue-600 text-white" : "text-white/70 hover:text-white"}`}
          ...
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                aria-label={captionsEnabled ? "Turn off captions" : "Turn on captions"}
              >
                <Subtitles className={`w-5 h-5 ${captionsEnabled ? "text-blue-400" : ""}`} />
              </button>

              <div className="flex items-center ml-6 relative volume-container">
                <button
                  className="text-white p-2.5 rounded-full hover:bg-white/20 transition-all duration-200 backdrop-blur-sm bg-black/20"
                  onClick={toggleMute}
                ...
                <div
                  className={`ml-3 transition-all duration-200 ${showVolumeSlider ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-2"}`}
              ...
              <button
                className="text-white p-2.5 rounded-full hover:bg-white/20 transition-all duration-200 backdrop-blur-sm bg-black/20 ml-6"
                onClick={toggleFullscreen}
                aria-label={isFullscreen ? "Exit fullscreen" : "Enter fullscreen"}
              >
            ...
            <div className="relative">
              ...
              {showSettings && (
                <div
                  ref={settingsRef}
                  className="absolute right-0 bottom-full mb-2 bg-black/90 backdrop-blur-md rounded-lg p-4 w-64 z-10 border border-white/10 shadow-xl"
                >
                  <div className="flex mb-4 space-x-1 bg-white/10 rounded-lg p-1">
                    <div
                      className={`settings-tab flex-1 text-center py-2 rounded-md transition-all ${activeSettingsTab === "quality" ? "bg-blue-600 text-white" : "text-white/70 hover:text-white hover:bg-white/10"}`}
                      onClick={() => setActiveSettingsTab("quality")}
                    >
                      Quality
                    </div>
                    <div
                      className={`settings-tab flex-1 text-center py-2 rounded-md transition-all ${activeSettingsTab === "speed" ? "bg-blue-600 text-white" : "text-white/70 hover:text-white hover:bg-white/10"}`}
                      onClick={() => setActiveSettingsTab("speed")}
                    >
                      Speed
                    </div>
                  ...
                  {activeSettingsTab === "quality" && (
                    <div className="grid grid-cols-2 gap-2">
                      {displayQualities.map((quality) => (
                        <button
                          key={quality}
                          className={`text-sm py-2.5 px-3 rounded-md transition-all ${
                            currentQuality === quality
                              ? "bg-blue-600 text-white shadow-lg"
                              : "bg-white/10 text-white/80 hover:bg-white/20 hover:text-white"
                          }`}
