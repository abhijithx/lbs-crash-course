    ...
    if (searchTerm.trim() === "") {
      ...
      const filtered = quizzes.filter(
        (quiz) => 
          ...
          (quiz.description && quiz.description.toLowerCase().includes(lowercasedSearch))
      )
    ...
    <div className="space-y-6">
      ...
      {filteredQuizzes.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-muted-foreground">No quizzes match your search criteria.</p>
          ...
          {filteredQuizzes.map((quiz) => {
            ...
            return (
              <Card key={quiz.id} className="hover:shadow-md transition-shadow">
                ...
                <CardFooter>
                  {isAdmin ? (
                    <Link href={`/crash-course/dashboard/quiz/groups/${groupId}/quizzes/${quiz.id}/edit`} className="w-full">
                      <Button variant="outline" className="w-full">
                        {status === "upcoming" ? "Edit Quiz" : "View Results"}
                      </Button>
                    </Link>
                  ) : (
                    <Link href={`/crash-course/dashboard/quiz/groups/${groupId}/quizzes/${quiz.id}`} className="w-full">
